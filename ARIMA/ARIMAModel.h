...
public:
    ...
    std::vector<double> preFirDiff(std::vector<double> preData){ //一阶差分(1)
    ...
    std::vector<double> preSeasonDiff(std::vector<double> preData){ //季节性差分(6, 7)
        std::vector<double> res;
    ...
    std::vector<double> preDealDiff(int period){
        if(period>=dataArray.size()-1){  // 将6也归为季节性差分
            period=0;
        }
    ...
    std::vector<int> getARIMAModel(int period,std::vector<std::vector<int> > notModel,bool needNot){
        ...
        for (int i = 0; i < cnt; ++i)
        {
            ...
            if (aic<=1.7976931348623157E308 && !std::isnan(aic) && aic < minAIC)
            {
                ...
                //std::cout<<minAIC<<' '<<bestModel[0]<<' '<<bestModel[1]<<std::endl;
                this->arima = coe;
            }
        }
        return bestModel;
